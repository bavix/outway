...
func TestNewPFBackend(t *testing.T) {
	t.Parallel()

	backend := firewall.NewPFBackend()
	// Backend might be nil if pfctl or route commands are not available
	if backend != nil {
		assert.Equal(t, "pf", backend.Name())
	}
}

func TestPFBackendName(t *testing.T) {
	t.Parallel()
...
func TestPFBackendMarkIP(t *testing.T) {
	t.Parallel()

	backend := firewall.NewPFBackend()
	if backend == nil {
		t.Skip("PF backend not available (pfctl or route commands not found)")
	}
	...
	// This might fail in test environment due to missing pfctl command
...
func TestPFBackendMarkIPInvalidInputs(t *testing.T) {
	t.Parallel()
...
func TestPFBackendMarkIPTTLNormalization(t *testing.T) {
	t.Parallel()
...
func TestPFBackendCleanupAll(t *testing.T) {
	t.Parallel()
...
func TestPFBackendConcurrency(t *testing.T) {
	t.Parallel()
...
func TestPFBackendIPv6(t *testing.T) {
	t.Parallel()
...
func TestPFBackendDifferentInterfaces(t *testing.T) {
	t.Parallel()
...
//nolint:funlen
func TestPFBackendErrorHandling(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				...
				// Even if it doesn't error, it might fail due to missing pfctl command
...
func TestPFBackendStressTest(t *testing.T) {
	t.Parallel()
...
func TestPFBackendTimerCleanup(t *testing.T) {
	t.Parallel()
	...
	// Test that timers are properly managed
	ip := "192.168.1.1"
	ttl := 1 // Very short TTL for testing

	// Add IP
	err := backend.MarkIP(ctx, "eth0", ip, ttl)
	_ = err
...
func TestPFBackendMultipleIPs(t *testing.T) {
	t.Parallel()
